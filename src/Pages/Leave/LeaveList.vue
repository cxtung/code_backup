...
<div class="table-responsive">
  <table>
    ...
    <tbody>
      <tr v-for="(leaves, index) in paginatedData" :key="index">
        <td>{{ leaves.date }}</td>
        <td>{{ leaves.employeeName }}</td>
        <td>{{ leaves.leaveBalanceDays }}</td>
...
import FilterLeave from "../../components/FilterLeave.vue";
...
export default {
  ...
  computed: {
    totalPages() {
      return Math.ceil(this.leaves.length / this.rowsPerPage);
    },
    paginatedData() {
  ...
  methods: {
    ...
    async fetchData() {
      ...
      try {
        const response = await axios.get(`${Apiurl}leave-summaries`, {
          params: {
        ...
        console.log("Data loaded:", this.leaves);
      } catch (error) {
